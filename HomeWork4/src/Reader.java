public class Reader {
    ...
    public static void PrintStatus(Reader reader) {
        ...
        for (int i = 0; i < reader.books.length; i++) {
            if (reader.books[i] == null) {
                ...
                System.out.print(reader.books[i].getTitle() + ", ");
            }
        }
        System.out.println();
    }
        ...
        if (book.getTitle().isEmpty()) {
            ...
            for (int i = 0; i < reader.books.length; i++) {
                if (reader.books[i] == null) {
                    ...
                    reader.booksCount++;
                    reader.WriteFullName();
                    System.out.println(" взял(а) книгу " + reader.books[i].getTitle());
                    break;
                }
            }
        }
    ...
    public static void returnBook(Reader reader, String title) {
        ...
        if (title.isEmpty()) {
            ...
            for (int i = 0; i < reader.books.length; i++) {
                if (reader.books[i] != null && reader.books[i].getTitle().equals(title)) {
                    reader.books[i] = null;
                    reader.booksCount--;
                    isExists = true;
                    reader.WriteFullName();
